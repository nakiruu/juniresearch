...
SLOTS="$(grep -oE 'cronTimesET: \[[^]]*\]' "$REPO/lib/trade/config.ts" | head -1 | grep -oE '[0-9]{2}:[0-9]{2}' | tr '\n' ' ')"
[ -n "$SLOTS" ] || { echo "error: could not read cronTimesET from lib/trade/config.ts" >&2; exit 1; }
CRON_TIME_ET="${SLOTS%% *}"   # first slot (== cronTimeET)
CAL_LINES=""; for t in $SLOTS; do CAL_LINES="${CAL_LINES}OnCalendar=Mon..Fri *-*-* ${t}:00 America/New_York
"; done
CAL="Mon..Fri *-*-* ${CRON_TIME_ET}:00 America/New_York"   # systemd >= 252 honors the timezone suffix (probe)
...
case "$mode" in
  ...
  --cron)
    ...
    for t in $SLOTS; do
      HH="${t%%:*}"; MM="${t##*:}"
      LINES="${LINES}$((10#$MM)) $((10#$HH)) * * 1-5 cd $REPO && PATH=$NODEBIN:\$PATH $NPM run trade:cron >> $REPO/data/trade/cron.log 2>&1 # ${UNIT}
"
    done
    ( crontab -l 2>/dev/null | grep -v "# ${UNIT}$"; printf "%s" "$LINES" ) | crontab -
  ...
  systemd)
    ...
    if ! systemd-analyze calendar "$CAL" >/dev/null 2>&1; then
      ...
      CAL_LINES=""; for t in $SLOTS; do CAL_LINES="${CAL_LINES}OnCalendar=Mon..Fri *-*-* ${t}:00
"; done
    fi
...
[Unit]
Description=Run Juniper trade:cron each trading day (${SLOTS% } ET)

[Timer]
$CAL_LINES# Persistent=false mirrors the Windows job: a missed ${CRON_TIME_ET} is SKIPPED, never run late at a worse
...
EOF
    ...
    echo "Installed systemd --user timer '${UNIT}.timer' → npm run trade:cron in $REPO (slots: ${SLOTS% } ET)."
