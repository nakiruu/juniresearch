...
function StepperTrigger({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLButtonElement>) => {
    // Our index among the registered triggers, read from the event target.
    const myIndex = () => triggerNodes.indexOf(e.currentTarget)
    switch (e.key) {
      case "ArrowRight":
      case "ArrowDown":
        e.preventDefault()
        { const i = myIndex(); if (i !== -1 && focusNext) focusNext(i) }
        break
      case "ArrowLeft":
      case "ArrowUp":
        e.preventDefault()
        { const i = myIndex(); if (i !== -1 && focusPrev) focusPrev(i) }
        break
      case "Home":
        e.preventDefault()
